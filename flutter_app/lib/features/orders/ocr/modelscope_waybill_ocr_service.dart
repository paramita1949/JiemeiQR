...
- merchantName: 提取“收货方业务短称”（不要公司全称），优先包含这些结尾或关键词：商贸、贸易、日用、供应链、美妆、百货、经贸、物流、集团、仓；也可识别“十足”“十足台州”“十足诸暨”这类门店/区域短称。读不到则空字符串
...
- merchantName 提取规则（仅通过语义，不要机械截断）：
  ...
  - 优先保留这些词：商贸、贸易、日用、供应链、美妆、百货、经贸、物流、集团、仓
  - 支持“十足”系列短称：十足、十足台州、十足诸暨
  - 示例：`浙江十足供应链管理有限公司` -> `十足供应链`；`台州某某商贸有限公司` -> `某某商贸`
- 读不清的字段返回空字符串或0，并在warnings用中文写原因
返回 JSON 对象，字段必须包含：
waybillNo, merchantName, orderDate, rows, warnings。
''';
