...
typedef OpenRouterHttpPost = Future<String> Function(
  Uri uri,
  Map<String, Object?> body,
  String apiKey,
);

class OpenRouterWaybillOcrService implements WaybillPhotoOcrService {
  OpenRouterWaybillOcrService({
    String? apiKey,
    String? model,
    FileAiConfigStore? configStore,
    OpenRouterHttpPost? httpPost,
  })  : apiKey = apiKey ?? const String.fromEnvironment('OPENROUTER_API_KEY'),
        model = model ?? const String.fromEnvironment('OPENROUTER_MODEL'),
  ...
  final OpenRouterHttpPost _httpPost;
  ...
  WaybillOcrDraft _parseResponse(String responseText) {
    ...
    if (decoded is! Map<String, Object?>) {
      throw const OpenRouterWaybillOcrException('OpenRouter 返回格式无效');
    }
    final choices = decoded['choices'];
    if (choices is! List || choices.isEmpty) {
      throw const OpenRouterWaybillOcrException('OpenRouter 未返回识别结果');
    }
    final first = choices.first;
    if (first is! Map) {
      throw const OpenRouterWaybillOcrException('OpenRouter 返回内容无效');
    }
    final message = first['message'];
    if (message is! Map) {
      throw const OpenRouterWaybillOcrException('OpenRouter 返回内容为空');
    }
    final content = message['content']?.toString().trim() ?? '';
    if (content.isEmpty) {
      throw const OpenRouterWaybillOcrException('OpenRouter 返回文本为空');
    }
    final payload = jsonDecode(content);
    if (payload is! Map<String, Object?>) {
      throw const OpenRouterWaybillOcrException('OCR JSON 格式无效');
    }
    return WaybillOcrDraft.fromJson(payload);
  }
}

class OpenRouterWaybillOcrException implements Exception {
  const OpenRouterWaybillOcrException(this.message);
...
Future<String> _defaultHttpPost(
  ...
  try {
    ...
    request.headers.set('HTTP-Referer', 'https://jiemei.local');
    request.headers.set('X-Title', 'Jiemei QR');
